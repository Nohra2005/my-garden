'use client';
// src/components/panels/LeadershipPanel.tsx — Leadership & Activities only
...
export function LeadershipPanel({ experiences, isOpen, onClose }: Props) {
  const items = [...experiences]
    .filter(e => e.type === 'leadership')
    .sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());

  return (
    <PanelBase section={CORRIDOR_SECTIONS.find(s => s.id === 'leadership')!} isOpen={isOpen} onClose={onClose}>
      {items.length === 0 && (
        <p style={{ color: SL, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 13 }}>
          No leadership entries yet — add them in Sanity Studio under 🌱 Leadership & Activities.
        </p>
      )}
